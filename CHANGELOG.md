...
* [CHANGE] Store-gateway: Raised the default value of `-blocks-storage.bucket-store.partitioner-max-gap-bytes` from 512 KiB to 4 MiB. Larger gaps consolidate more series into a single object-storage GET, reducing goroutine count and mutex contention on the hot `loadSeries` path under high fan-out, at the cost of fetching a small amount of unused bytes per merged range. Cells with sparse access patterns or tight bandwidth budgets that prefer the prior behaviour can set the flag back to `524288`. #TBD
...
* [ENHANCEMENT] Store-gateway: Replace the bespoke `sync.Map`-based dedup of `ExpandedPostings` with a `golang.org/x/sync/singleflight` group, and apply the same dedup to `LabelNames`, `LabelValues`, and the `ExpandedPostings` populate path. Concurrent identical queries that miss the cache now share one compute+store pass; the leader's compute runs to completion regardless of any individual caller's context cancellation, fixing a long-standing TODO where a cancelled originator could poison waiting followers. #TBD
